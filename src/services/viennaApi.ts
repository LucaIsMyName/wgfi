 ...
 * Includes all possible field names that may appear in the API response
 */
...
function parseViennaCSV(csvText: string): ViennaPark[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const park: ViennaPark = {
      ...
      properties: {
        OBJECTID: parseInt(row.OBJECTID || row.FID || String(i)),
...
export function transformViennaPark(viennaPark: ViennaPark) {
  ...
  const result = {
    ...
    openingHours: formatOpeningHours(props.OEFF_ZEITEN || '') || 'Täglich geöffnet',
  ...
  // Attach districtAreaSplit from manual data if available
  if (manualData?.districtAreaSplit) {
    (result as any).districtAreaSplit = manualData.districtAreaSplit;
  }
...
function transformManualPark(key: string, manualPark: ManualParkData) {
  ...
  // Attach districtAreaSplit if provided
  if (manualPark.districtAreaSplit) {
    (result as any).districtAreaSplit = manualPark.districtAreaSplit;
  }
...
// Local storage keys
const PARKS_STORAGE_KEY = 'wgfi:parks-data';
const PARKS_TIMESTAMP_KEY = 'wgfi:parks-timestamp';
